  ...
  ia: {
    ...
    nodes: [
      ...
      },
      {
        ...
        lessons: [
          {
            title: "Pourquoi ça compte pour l'embauche",
            paragraphs: [
              "Le prompt engineering est la compétence IA la plus demandée en entreprise en ce moment — mais 'savoir utiliser ChatGPT' ne suffit plus aux recruteurs. Ce qu'ils veulent voir, c'est ta capacité à obtenir un résultat exploitable directement, pas à bricoler pendant 10 tentatives.",
              "Un bon prompt engineer, c'est quelqu'un qui transforme une idée floue ('fais-moi un truc sur X') en instruction précise qui donne un résultat utilisable du premier coup — ou presque.",
            ],
          },
          {
            title: "Les 4 piliers d'un bon prompt",
            paragraphs: [
              "Le contexte : qui tu es, pour qui c'est, dans quel but précis. Sans ça, l'IA devine — et devine souvent mal.",
              "Le format attendu : longueur, structure (liste, tableau, JSON, code), niveau de détail. Précise-le toujours, l'IA ne le devine pas correctement seule.",
              "Le ton : formel, décontracté, adapté à un public jeune, professionnel... Ça change complètement le résultat.",
              "Un exemple concret : montrer une fois ce que tu attends vaut mieux que trois paragraphes d'explication.",
            ],
          },
          {
            title: "Les erreurs qui ruinent un prompt",
            paragraphs: [
              "Trop vague : 'génère des questions sur la Bible' donne un résultat générique et inutilisable tel quel.",
              "Aucun format précisé : tu récupères un texte informe qu'il faut tout retravailler à la main.",
              "Une seule tentative : le prompting est itératif — tu affines en fonction de ce que l'IA renvoie, exactement comme une conversation de travail avec un collègue.",
              "Ne pas vérifier le résultat : une IA peut se tromper avec assurance (on appelle ça une hallucination). Le jugement humain qui valide le résultat, c'est justement ce qui distingue un professionnel d'un simple utilisateur d'IA.",
            ],
          },
        ],
        exercise: {
          type: "ai-prompt",
          prompt:
            "Écris un prompt pour demander à une IA de générer 5 questions de quiz biblique (QCM) sur le livre de la Genèse, destinées à des jeunes de 15-18 ans. Applique les 4 piliers : contexte, format, ton, exemple.",
        },
        resource: {
          title: "Modèle de prompt réutilisable",
          content:
            "Contexte : Je suis [ton rôle], je crée [type de contenu] pour [public cible].\nTâche : Génère [quantité] de [élément précis] sur le thème [sujet].\nFormat : [structure exacte attendue — liste, JSON, tableau...]\nTon : [adapté à ton public]\nExemple attendu : [1 exemple de ce que tu veux obtenir]\n\nCopie cette structure et adapte les crochets à chaque nouveau projet.",
        },
      },
      {
        id: "ia-4",
    ...
    nodes: [
      {
        ...
        lessons: [
          {
            title: "Pourquoi ça compte pour l'embauche",
            paragraphs: [
              "85% des recruteurs exigent un portfolio, mais le jugement sur les couleurs, c'est ce qui distingue un vrai graphiste d'un simple utilisateur d'IA — n'importe qui peut générer un visuel, peu de gens savent dire précisément pourquoi il fonctionne ou pas.",
              "Cette leçon est théorique, mais l'exercice à la fin se fait en vrai sur Canva ou Figma — c'est cette pratique réelle (au moins 50% de ton temps d'apprentissage) que les recruteurs regardent dans un portfolio.",
            ],
          },
          {
            title: "Le cercle chromatique et les 3 harmonies",
            paragraphs: [
              "Le cercle chromatique organise les couleurs et permet de construire des harmonies fiables, au lieu de choisir des couleurs au hasard.",
              "Harmonie complémentaire : deux couleurs opposées sur le cercle (ex: bleu/orange) — fort contraste, très dynamique. Idéal pour un bouton d'action ou une affiche qui doit attirer l'œil immédiatement.",
              "Harmonie analogue : couleurs voisines sur le cercle (ex: bleu, bleu-vert, vert) — douce et cohérente. Idéal pour une interface calme ou un thème spirituel/bien-être.",
              "Harmonie triadique : trois couleurs équidistantes sur le cercle — équilibrée et vive, à utiliser avec une couleur dominante. Idéal pour du contenu jeunesse ou ludique.",
            ],
          },
          {
            title: "La règle 60-30-10",
            paragraphs: [
              "Une palette pro ne mélange jamais 5 couleurs à parts égales — elle suit une hiérarchie : 60% couleur dominante (le fond, l'ambiance générale), 30% couleur secondaire (les blocs de contenu), 10% couleur d'accent (les boutons, ce qui doit sauter aux yeux).",
              "C'est exactement ce qui manque dans les palettes 'amateur' : tout crie en même temps, rien ne ressort. La règle 60-30-10 force une hiérarchie claire, comme sur l'affiche qu'on a analysée ensemble (violet dominant, blanc secondaire, doré en accent).",
            ],
          },
        ],
        exercise: {
        ...
        resource: {
          title: "Fiche mémo — palette 60-30-10",
          content:
            "60% Dominante : ta couleur de fond, elle donne l'ambiance générale\n30% Secondaire : pour les blocs de contenu, les cartes, les sections\n10% Accent : réservée aux boutons et éléments qui doivent capter l'attention\n\nÀ faire maintenant sur Canva/Figma : choisis un thème (ex: une affiche d'événement), applique cette règle avec 3 couleurs issues d'une des 3 harmonies vues dans la leçon.",
        },
      },
      {
        id: "d-2",
    ...
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center">
      ...
      >
        ...
        <div className="px-5 py-5">
          ...
          {step === "result" && <ResultView accent={track.accent} onClose={onClose} resource={node.resource} />}
        </div>
      </div>
    </div>
  );
...
function LessonView({ node, accent, onNext }) {
  // Nouveau format : plusieurs leçons { title, paragraphs }
  if (node.lessons) {
    const [pageIndex, setPageIndex] = useState(0);
    const page = node.lessons[pageIndex];
    const isLast = pageIndex === node.lessons.length - 1;
    return (
      <div>
        <div className="text-xs text-white/40 mb-3">Leçon {pageIndex + 1} / {node.lessons.length}</div>
        <div className="text-white font-bold text-lg display-font mb-4">{page.title}</div>
        ...
        <button
          onClick={() => (isLast ? onNext() : setPageIndex(pageIndex + 1))}
          className="w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2 text-[#0B1512]"
          style={{ background: accent }}
        >
          {isLast ? "Passer à l'exercice" : "Leçon suivante"} <ChevronRight size={16} />
        </button>
      </div>
    );
  }

  // Ancien format : une seule leçon (tableau de paragraphes)
  return (
    <div>
      <div className="space-y-4 mb-6">
        {node.lesson.map((p, i) => (
...
function ResultView({ accent, onClose, resource }) {
  ...
  return (
    <div className="text-center py-6">
      ...
      {resource && (
        <div className="text-left mb-6 rounded-xl overflow-hidden" style={{ border: `1px solid ${accent}40` }}>
          <div className="px-4 py-2 flex items-center gap-2" style={{ background: accent + "20" }}>
            <span>🎁</span>
          ...
          <div className="px-4 py-3" style={{ background: "rgba(255,255,255,0.03)" }}>
            <div className="text-sm font-semibold text-white mb-2">{resource.title}</div>
            <pre className="text-xs text-white/70 whitespace-pre-wrap leading-relaxed font-sans">{resource.content}</pre>
            <button
              onClick={() => {
